...
def evaluate(tf_graph, sess, indv_scope, therohold,adj_matrix, evaluate_repeat, max_iterations, evoluation_goal=None, evoluation_goal_square_norm=None):		
	with tf_graph.as_default():
		with tf.variable_scope(indv_scope):
			adj_matrix_in=adj_matrix.copy()
			...
			step = tf.train.AdamOptimizer(0.01).minimize(rse_loss, var_list=var_list)
			var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=indv_scope)
		...
		for r in range(evaluate_repeat):
			...
			for i in range(max_iterations): 
				sess.run(step)
				if sess.run(rse_loss)<therohold:
								break
...
if __name__ == '__main__':
	...
	while True:
		...
		if flag:
			...
			try:
				repeat_loss = evaluate(tf_graph=g, sess=sess, indv_scope=scope, therohold=rse_TH,adj_matrix=adj_matrix, evaluate_repeat=repeat, max_iterations=iters,
